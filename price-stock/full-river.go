...
func query(bt *Mysqlbeat) {
	...
	var (
		url      = flag.String("url", "http://es.search.ickey.cn:9200", "Elasticsearch URL")
	...
	suffexs := [...]string{".es_v", ".pro_v", ".key_v", ".sql_v"}
	//suffexs := [...]string{".es_v", ".pro_v"}
	//suffexs := []string{".es_v", ".pro_v"}

	var myIndex string

	for _, suffex := range suffexs {
	...
	if false == b {
		fmt.Println("not inclue like *.es_v or *.pro_v return")
	...
	g.Go(func() error {
		...
		for d := range docsc {
			...
			if strings.EqualFold(in, "product") {
				//Enqueue the document
				productid, err := d.GetValue("pro_id")
				if err != nil {
					fmt.Println("id irrorerr %v", d)
					logp.Info("Es bulk err id:%v  ,  err=%v", id, err)
					continue
				}
				iid := productid.(int64)
				id = strconv.FormatInt(iid, 10)
			} else {
