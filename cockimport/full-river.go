...
func init() {
	...
	db = GetConnection()
	//##############################################mysqldb###############################################
	// db, _ = sql.Open("mysql", "root:ickey_2016@tcp(10.8.11.225:3306)/db_product?tls=skip-verify&timeout=5s")
	...
	//for searchits and orderhits
	//db, _ = sql.Open("mysql", "transit:PaywXFJyeJXzmmQt@tcp(mysqldb.product.rw.01.ickey.cn:3306)/db_erp")
	...
	if isDebug {
		...
		addr = "postgresql://stan@10.8.51.115:26257/db_product?sslcert=/usr/local/ickey-certs/client-stan/client.stan.crt&sslkey=/usr/local/ickey-certs/client-stan/client.stan.key&sslrootcert =/usr/local/ickey-certs/client-stan/ca.crt&sslmode=require"
	}
...
func (bt *Mysqlbeat) Query() {
	...
	var (
		fromdb = flag.String("fromdb", "t_pro_sell_stock", "from mysql")
		todb   = flag.String("todb", "", "to cockroach db name")
	)
	flag.Parse()
	log.SetFlags(0)
	...
	go Storage(cdb, ctx1, docsc)
	ret, lerr = bt.DoMysqlLimitQuery(ctx1, cancel1, sql, docsc, *todb)
	cancel1()
	select {
	case <-ctx1.Done():
		////finish range loop
		//server.close
		fmt.Println("first runnig. done.")
	}
	close(docsc)
	if lerr != nil {
	...
	if ret > 0 {
		for {
			...
			docsc1 := make(chan common.MapStr, 500)

			defer close(docsc1)
			ctx, cancel := context.WithCancel(context.Background())

			go Storage(cdb, ctx, docsc1)
			// fmt.Println("##mysql query ::::", sql)
			ret, lerr = bt.DoMysqlLimitQuery(ctx, cancel, sql, docsc1, *todb)
			if lerr != nil {
				log.Printf("db op reconnection  err%v", lerr)
				db = GetConnection()
			}
...
func (bt *Mysqlbeat) DoMysqlLimitQuery(ctx context.Context, cancel context.CancelFunc, sql string, docsc chan<- common.MapStr, todb string) (int, error) {
	var ret int
