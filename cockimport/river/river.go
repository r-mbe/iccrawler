...
import (
	...
	"techtoolkit.ickey.cn/cockimport/elastic"
)
...
func NewRiver(c *Config) (*River, error) {
	...
	//cockroach db module
	cockCfg := new(cockroach.ClientConfig)
	cockCfg.CockDBName = r.c.CockDBName
	cockCfg.CockAddr = r.c.CockAddr
	cockCfg.CockUser = r.c.CockUser
	cockCfg.CockPassword = r.c.CockPassword
	cockCfg.SeelogFile = r.c.SeelogFile
	r.cock = cockroach.NewClient(cockCfg)
