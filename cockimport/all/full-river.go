...
import (
	...
	"techtoolkit.ickey.cn/cockimport/nsqproducer"
)
...
func (d *Dispatcher) dispatch() {
	for {
		select {
		case <-d.ctx.Done():
			fmt.Println("end dispatch game over.")
			return
		case job := <-JobQueue:
			...
			go func(job *Job) {
				...
				// fmt.Println("Before sender to nsq", string(body))
				// fmt.Println("table=", job.Payload.Event["table"].(string))
				if strings.EqualFold(job.Payload.Event["table"].(string), "t_pro_sell_price") {
					//Enqueue the document
					m := new(Message)
					p := new(TProSellPrice)
					err := mysql2Cock(job.Payload.Event, p)
					if err != nil {
						fmt.Println("mysql convert doc error")
					} else {
						if p.MysqlID == 0 {
							//un useif id.
							return
						...
						if err != nil {
							fmt.Println("Err Dispater Mapstr to []byte", err, job)
							return
						}

						m.Cmd = "c_price"
						m.Data = data
						// fmt.Println("XXXXXBefore send []byte", string(data))

						json, err := json.Marshal(m)
						if err != nil {
							fmt.Println("Marshal msg to []byte", err, json)
							return
						}

						err = d.S.Sender(json)
						if err != nil {
							fmt.Println("Err Dispater Nsq send err", err, job)
						}
					}
				}
				//send job to nsq msg queue!
			}(job)
		}
	}
}

func mysql2Cock(d common.MapStr, price *TProSellPrice) (ret error) {
	...
	if v, ok := d["last_update_time"]; ok {
		...
		price.LastUpTime = time.Now().Unix()
	}
...
func main() {
	...
	bt := New()
	bt.Query(*workers, *queues, *fromdb, *todb)
...
func (bt *Mysqlbeat) generateEventFromRow(row *sql.Rows, columns []string, queryType string, rowAge time.Time) (common.MapStr, error) {
	...
	for i, col := range values {
		...
		if strings.EqualFold("pro_sno", strColName) ||
		...
		} else if strings.EqualFold("price1", strColName) ||
			strings.EqualFold("price2", strColName) ||
			strings.EqualFold("price3", strColName) ||
			strings.EqualFold("price4", strColName) ||
			strings.EqualFold("price5", strColName) ||
			strings.EqualFold("price6", strColName) ||
			strings.EqualFold("price7", strColName) ||
			strings.EqualFold("price8", strColName) ||
			strings.EqualFold("price9", strColName) ||
			strings.EqualFold("price10", strColName) {
			...
			if err != nil {
				...
				// fmt.Println(" Mysql conver column err", err, strColName, strColType, strColName)
				continue
			}
			...
			if fColValue >= -0.0000001 && fColValue < 0.0000001 {
				fColValue = 0.000000
			}
			// fmt.Println("after convert col= 0.3f", fColValue)
...
func (bt *Mysqlbeat) Query(workers int, queues int, fromdb string, todb string) {
	...
	for i, doc := range docs {
		...
		JobQueue <- &Job{&Payload{Name: fmt.Sprintf("%d", jid), Event: doc}}
	}
	// dispatcher.Stop()
	defer d.Stop()
